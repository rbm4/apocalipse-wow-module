...
| `data/sql/db-world/2026_09_26_01_threat_of_thassarian_healing.sql` | `acore_world` | AzerothCore module updater; rerunnable exact binding plus guarded talent-row updates | Binds the stock Death Strike handler to off-hand rank chain 66188 and updates all three talent descriptions for independent off-hand healing and a 30 percent reduction per heal |
...
| `data/sql/db-world/2026_09_26_00_death_knight_rupture_melee_hits.sql` | `acore_world` | AzerothCore module updater; ID-scoped and idempotent for existing 901048 rows | Uses the outgoing-hit default proc mask so normal, critical, and absorbed melee events qualify, preserves 100 percent chance with no cooldown or charges, and updates client-export descriptions |
...
The Threat of Thassarian baseline copies complete stock rows 65661, 66191, and 66192 from `wotlk_spells_full` into `spell_dbc` before changing descriptions. Helpers 901156 and 901157 are complete `spell_dbc` overrides with `wotlk_spells` names. The healing follow-up binds off-hand Death Strike rank chain 66188 to the shared core handler and updates the three existing overrides. The backend release builder consumes these overrides directly when producing `Spell.dbc`; it does not require custom rows to be inserted into `wotlk_spells_full`. `Talent.dbc` remains unchanged.
