...
Owners: `src/mod_apocalipse_warlock_demonic_equilibrium.cpp`, `data/sql/db-world/2026_09_20_06_demonic_equilibrium.sql`, `src/mod_apocalipse_loader.cpp`

Last source review: 2026-09-20

## Purpose

Demonic Equilibrium is a custom Warlock passive using spell ID 901033. While the passive and stock Soul Link aura 25228 are active, 75 percent of incoming damage is transferred to the controlled demon instead of Soul Link's stock 20 percent.
...
| Server migration | `data/sql/db-world/2026_09_20_06_demonic_equilibrium.sql` |
...
Warlock with active Soul Link takes damage
  ...
  -> when present, replace the split amount with 75 percent of current incoming damage
...
The 75 percent is calculated from the damage visible to Soul Link at its normal place in `Unit::CalcAbsorbResist`. Earlier absorb, resistance, and split processing continues to affect that input according to core ordering.

## Server spell contract

The automatic world update defines 901033 as an infinite passive Warlock dummy aura using Soul Link's icon and Shadow school. It collision-checks `spell_dbc`, `wotlk_spells_full`, and `wotlk_spells`, binds the AuraScript to stock split aura 25228, and synchronizes the backend spell name.
...
| Soul Link and passive 901033 active with a living controlled demon | 75 percent of eligible incoming damage transfers to the demon | Not run |
| Passive learned while Soul Link is already active | The next qualifying hit uses 75 percent without a Soul Link recast | Not run |
...
- A 75 percent transfer can kill the demon quickly and requires in-game balance validation.
