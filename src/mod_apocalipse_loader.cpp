/*
 * mod_apocalipse_loader.cpp
 *
 * AzerothCore module entry-point.
 * The build system converts the folder name (apocalipse-wow-module) -
 * dashes become underscores - so this function MUST match exactly.
 */

void AddModApocalipseScripts();
void AddModSpellScalingScripts();
void AddModApocalipsePvPScripts();
void AddModApocalipseMageSpellScripts();
void AddModApocalipseMagePyroclasticChainReactionScripts();
void AddModApocalipseMageMissileBarrageOverloadScripts();
void AddModApocalipseMageHypernovaScripts();
void AddModApocalipseMagePrismaticBarrierScripts();
void AddModApocalipseMageFrostBombScripts();
void AddModApocalipseMageAutomaticIceLanceScripts();
void AddModApocalipseMageFrozenRetaliationScripts();
void AddModApocalipseRogueConcentratedVenomScripts();
void AddModApocalipseRogueLeechingMixtureScripts();
void AddModApocalipseRogueGloombladeInfusionScripts();
void AddModApocalipseRogueShadowExecutionScripts();
void AddModApocalipseRogueRelentlessFinaleScripts();
void AddModApocalipseRogueImprovedFeintScripts();
void AddModApocalipseRogueDaringChallengeScripts();
void AddModApocalipseRogueBladeguardScripts();
void AddModApocalipseRogueShieldedReflexesScripts();
void AddModApocalipseRogueBucklerStrikeScripts();
void AddModApocalipseHunterAmbushTrapperScripts();
void AddModApocalipseHunterApexBondScripts();
void AddModApocalipseHunterBloodOfTheHuntScripts();
void AddModApocalipseHunterPrimalResolveScripts();
void AddModApocalipseRogueAlchemicalGuardScripts();
void AddModApocalipseDeathKnightCrimsonWardScripts();
void AddModApocalipseDeathKnightFrozenResolveScripts();
void AddModApocalipseDeathKnightNecroticVeilScripts();
void AddModApocalipseDeathKnightPestilentEruptionScripts();
void AddModApocalipseDeathKnightThreatOfThassarianScripts();
void AddModApocalipseDeathKnightRimeShardsScripts();
void AddModApocalipseDeathKnightRuptureScripts();
void AddModApocalipseRoguePestilentKnivesScripts();
void AddModApocalipsePaladinDivineStormEchoScripts();
void AddModApocalipsePaladinPermanentSealOfRighteousnessScripts();
void AddModApocalipsePaladinDivineTollScripts();
void AddModApocalipsePaladinDivineSteedScripts();
void AddModApocalipseWarlockBurningConflagrationScripts();
void AddModApocalipseWarlockChaoticInfernoScripts();
void AddModApocalipseWarlockDemonicEquilibriumScripts();
void AddModApocalipseWarlockHauntingAfflictionScripts();
void AddModApocalipseWarlockPermanentMetamorphosisScripts();
void AddModApocalipseShamanSpellScripts();
void AddModApocalipsePriestSpellScripts();
void AddModApocalipseBattlegroundStaminaScripts();

void Addapocalipse_wow_moduleScripts()
{
    AddModApocalipseScripts();
    AddModSpellScalingScripts();
    AddModApocalipsePvPScripts();
    AddModApocalipseMageSpellScripts();
    AddModApocalipseMagePyroclasticChainReactionScripts();
    AddModApocalipseMageMissileBarrageOverloadScripts();
    AddModApocalipseMageHypernovaScripts();
    AddModApocalipseMagePrismaticBarrierScripts();
    AddModApocalipseMageFrostBombScripts();
    AddModApocalipseMageAutomaticIceLanceScripts();
    AddModApocalipseMageFrozenRetaliationScripts();
    AddModApocalipseRogueConcentratedVenomScripts();
    AddModApocalipseRogueLeechingMixtureScripts();
    AddModApocalipseRogueGloombladeInfusionScripts();
    AddModApocalipseRogueShadowExecutionScripts();
    AddModApocalipseRogueRelentlessFinaleScripts();
    AddModApocalipseRogueImprovedFeintScripts();
    AddModApocalipseRogueDaringChallengeScripts();
    AddModApocalipseRogueBladeguardScripts();
    AddModApocalipseRogueShieldedReflexesScripts();
    AddModApocalipseRogueBucklerStrikeScripts();
    AddModApocalipseHunterAmbushTrapperScripts();
    AddModApocalipseHunterApexBondScripts();
    AddModApocalipseHunterBloodOfTheHuntScripts();
    AddModApocalipseHunterPrimalResolveScripts();
    AddModApocalipseRogueAlchemicalGuardScripts();
    AddModApocalipseDeathKnightCrimsonWardScripts();
    AddModApocalipseDeathKnightFrozenResolveScripts();
    AddModApocalipseDeathKnightNecroticVeilScripts();
    AddModApocalipseDeathKnightPestilentEruptionScripts();
    AddModApocalipseDeathKnightThreatOfThassarianScripts();
    AddModApocalipseDeathKnightRimeShardsScripts();
    AddModApocalipseDeathKnightRuptureScripts();
    AddModApocalipseRoguePestilentKnivesScripts();
    AddModApocalipsePaladinDivineStormEchoScripts();
    AddModApocalipsePaladinPermanentSealOfRighteousnessScripts();
    AddModApocalipsePaladinDivineTollScripts();
    AddModApocalipsePaladinDivineSteedScripts();
    AddModApocalipseWarlockBurningConflagrationScripts();
    AddModApocalipseWarlockChaoticInfernoScripts();
    AddModApocalipseWarlockDemonicEquilibriumScripts();
    AddModApocalipseWarlockHauntingAfflictionScripts();
    AddModApocalipseWarlockPermanentMetamorphosisScripts();
    AddModApocalipseShamanSpellScripts();
    AddModApocalipsePriestSpellScripts();
    AddModApocalipseBattlegroundStaminaScripts();
}
